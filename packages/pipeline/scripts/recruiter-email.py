...
def build_prompt(
    meta: dict, email_type: str, contact_name: str, job_text: str, research_text: str, cv_data: dict | None = None
) -> str:
...
def save_output(app_dir: Path, meta: dict, email_type: str, contact_name: str, raw_output: str, provider: str) -> Path:
...
def main():
    parser = argparse.ArgumentParser(description="Generate personalised recruiter outreach emails")
    parser.add_argument("app_dir", help="Application directory")
    parser.add_argument(
        "--type",
        choices=["cold", "follow-up", "post-apply"],
        default="cold",
        dest="email_type",
    ...
    prompt = build_prompt(meta, args.email_type, contact_name, job_text, research_text, cv_data=cv_data)
